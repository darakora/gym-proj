...
import styles from './GuashaMassagePage.module.scss';
...
export const GuashaMassagePage = () => {
  ...
  const routes = () => {
    if (data) {
      return [
        { id: 'guasha-massage', title: 'mein Bonusmaterial', path: '/' },
        {
          id: 'guasha-massage',
          title: `${data.title}`,
          path: `/guasha-massage/${data.id}`
        }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[3].links[0].id}`}
              isNextDisabled={id === `${links[3]?.links?.at(-1)?.id || ''}`}
              path="guasha-massage"
              id={data.id}
              links={links[3].links}
