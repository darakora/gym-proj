...
import styles from './EyeExercisePage.module.scss';
  ...
  const routes = () => {
    if (data) {
      return [
        ...
        { id: 'eye', title: `${data.title}`, path: `/augengym/${data.id}` }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[6].links[0].id}`}
              isNextDisabled={id === `${links[6]?.links?.at(-1)?.id || ''}`}
              path="augengym"
              id={data.id}
              links={links[6].links}
