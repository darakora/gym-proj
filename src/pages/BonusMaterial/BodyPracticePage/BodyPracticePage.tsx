...
import styles from './BodyPracticePage.module.scss';
    ...
    if (data) {
      return [
        { id: 'bonus', title: 'mein Bonusmaterial', path: '/' },
        {
          id: 'body',
          title: `${data.title}`,
          path: `/bodyPractices/${id as string}`
        }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {data && links && links[2].links.at(-1) && (
            <Navigation
              isPreviousDisabled={id === `${links[2].links[0].id}`}
              isNextDisabled={id === `${links[2]?.links?.at(-1)?.id || ''}`}
              path="bodyPractices"
              id={data.id}
              links={links[2].links}
