...
import styles from './TapticTechniquePage.module.scss';
...
export const TapticTechniquePage = () => {
  ...
  const routes = () => {
    if (data) {
      return [
        ...
        { id: 'taping', title: `${data.title}`, path: `/taping/${data.id}` }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[4].links[0].id}`}
              isNextDisabled={id === `${links[4]?.links?.at(-1)?.id || ''}`}
              path="taping"
              id={data.id}
              links={links[4].links}
