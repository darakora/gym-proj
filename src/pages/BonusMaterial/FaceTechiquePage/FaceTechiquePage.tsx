...
import styles from './FaceTechiquePage.module.scss';
  ...
  const routes = () => {
    if (data) {
      return [
        ...
        { id: 'face', title: `${data.title}`, path: `/facial/${data.id}` }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[5].links[0].id}`}
              isNextDisabled={id === `${links[5]?.links?.at(-1)?.id || ''}`}
              path="facial"
              id={data.id}
              links={links[5].links}
