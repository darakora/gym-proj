...
import styles from './RoutinePage.module.scss';
  ...
  const routes = () => {
    if (data) {
      return [
        ...
        { id: 'routine', title: `${data.title}`, path: `/routines/${data.id}` }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[1].links[0].id}`}
              isNextDisabled={id === `${links[1]?.links?.at(-1)?.id || ''}`}
              path="routines"
              id={data.id}
              links={links[1].links}
