...
import styles from './GuestsRecordingsPage.module.scss';
...
export const GuestsRecordingsPage = () => {
  ...
  const routes = () => {
    if (data) {
      return [
        { id: 'guest-record', title: 'mein Bonusmaterial', path: '/' },
        {
          id: 'guest-record',
          title: `${data.title}`,
          path: `/guest-record/${data.id}`
        }
      ];
    }
  };
    ...
    <>
      <div className={styles.container}>
        ...
        <div className={styles.content}>
          ...
          {links && data && (
            <Navigation
              isPreviousDisabled={id === `${links[8].links[0].id}`}
              isNextDisabled={id === `${links[8]?.links?.at(-1)?.id || ''}`}
              path="guest-record"
              id={data.id}
              links={links[7].links}
